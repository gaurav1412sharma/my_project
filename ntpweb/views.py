...
def favicon_view(request):
    ...
    favicon_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'build', 'favicon.ico')
    logger.debug(f"Serving favicon.ico from: {favicon_path}")
    try:
        with open(favicon_path, 'rb') as file:
            return HttpResponse(file.read(), content_type='image/x-icon')
    except Exception as e:
        logger.error(f"Error serving favicon.ico: {e}", exc_info=True)
        return HttpResponse(f"Error: {e}", status=500)